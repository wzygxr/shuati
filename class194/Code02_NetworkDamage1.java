...
// 查询要求：对于 q 条查询，每条查询给定三个节点 x、y 和 z
//           判断如果删除节点 z，x 和 y 是否会断连
// 数据范围：1 <= n <= 2 * 10^4，1 <= m <= 10^5，1 <= q <= 10^5
...
public class Code02_NetworkDamage1 {
	...
	// 最大节点数，根据题目数据范围设置为 20001
	public static int MAXN = 20001;
	// 最大边数，根据题目数据范围设置为 100001
	public static int MAXM = 100001;
	// 倍增表的最大幂次，2^18 足够覆盖 2e4 量级的数据
	...
	// 迭代版算法需要的栈结构（用于递归改迭代）
	// stack[][]：存储迭代过程中的状态，每个元素包含三个值：u（当前节点）、status（状态）、e（边索引）
	// u, status, e：当前迭代的状态变量
	...
	public static void tarjan2(int node) {
		...
		push(node, -1, -1);  // 将起始节点压入栈，状态为 -1（未处理）
		int v;
		// 迭代处理栈中的状态
		while (stacksize > 0) {
			pop();  // 弹出栈顶状态
	...
	public static void main(String[] args) throws Exception {
		...
		for (int i = 1, x, y, z; i <= q; i++) {
			...
			y = in.nextInt();
			z = in.nextInt();
			// 如果 x 到 y 的距离等于 x 到 z 的距离加上 z 到 y 的距离
			// 说明 z 在 x 到 y 的所有路径上，删除 z 会导致 x 和 y 断连
