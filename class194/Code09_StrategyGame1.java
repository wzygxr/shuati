...
public class Code09_StrategyGame1 {
	...
	// 自定义排序函数，按照圆方树的 dfn 序号对节点进行排序
	// nums：待排序的节点数组
	// l：排序区间的左端点
	// r：排序区间的右端点
	public static void sort(int[] nums, int l, int r) {
		if (l >= r) return;  // 如果区间长度为 0 或 1，直接返回
		int i = l, j = r;
		int pivot = nums[(l + r) >> 1];  // 选择中间元素作为基准
		while (i <= j) {
			// 找到第一个 dfn 序号大于等于基准的节点
			while (nid[nums[i]] < nid[pivot]) i++;
			// 找到最后一个 dfn 序号小于等于基准的节点
			while (nid[nums[j]] > nid[pivot]) j--;
			if (i <= j) {  // 如果 i <= j，交换两个节点
				int tmp = nums[i]; nums[i] = nums[j]; nums[j] = tmp;
				i++;
				j--;
			}
		}
		sort(nums, l, j);  // 递归排序左半部分
		sort(nums, i, r);  // 递归排序右半部分
	}

	// 计算当前查询的答案
	// 返回值：可以选择的点的个数
	public static int compute() {
		sort(arr, 1, s);  // 按照圆方树的 dfn 序号对查询节点进行排序
		int sumDist = 0;
		// 计算相邻节点之间的圆点数量之和
		for (int i = 1; i < s; i++) {
	...
	public static void main(String[] args) throws Exception {
		...
		for (int c = 1; c <= t; c++) {
			...
			// 处理 q 条查询
			for (int i = 1; i <= q; i++) {
				s = in.nextInt();  // 读取当前查询的节点数
				// 读取当前查询的 s 个节点
				for (int j = 1; j <= s; j++) {
					arr[j] = in.nextInt();
				}
				out.println(compute());  // 计算并输出答案
			}
		}
		out.flush();  // 刷新输出缓冲区
		out.close();  // 关闭输出流
