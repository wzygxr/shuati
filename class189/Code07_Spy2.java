...
/**
 * 【C++版本代码】间谍网络问题
 * 核心思路：从可贿赂的间谍开始DFS，找到所有可达的间谍
 * 如果存在不可达的间谍，输出NO
 * 否则计算入度为0的SCC的最小贿赂价格之和
 */
...
// int n, p, m;
// 
// int cost[MAXN], a[MAXM], b[MAXM];
// int head[MAXN], nxt[MAXM], to[MAXM], cntg;
// int dfn[MAXN], low[MAXN], cntd;
// int sta[MAXN], top;
// int belong[MAXN], minVal[MAXN], sccCnt;
// int indegree[MAXN];
// 
...
//     
//     for (int i = 1; i <= n; i++) {
//         if (cost[i] != INF && dfn[i] == 0) tarjan(i);
//     }
//     
//     bool check = true;
//     int ans = 0;
