...
public class Code07_SoftwareInstallation1 { // 定义公共类Code07_SoftwareInstallation1

	public static int MAXN = 301; // 定义最大节点数常量，最多300个节点（考虑虚拟根节点）
	public static int MAXM = 601; // 定义最大容量常量，最多600
	public static int n, m; // n为物品数，m为背包容量
	public static int[] w = new int[MAXN]; // 存储每个物品的重量
	public static int[] v = new int[MAXN]; // 存储每个物品的价值
	public static int[] depend = new int[MAXN]; // 存储每个物品的依赖物品编号，0表示无依赖

	public static int[] head = new int[MAXN]; // 邻接表的头指针数组
	public static int[] nxt = new int[MAXN]; // 邻接表的next数组
	public static int[] to = new int[MAXN]; // 邻接表的to数组
	...
	public static int[] wsum = new int[MAXN]; // 记录每个SCC的总重量（循环依赖的物品重量和）
	public static int[] vsum = new int[MAXN]; // 记录每个SCC的总价值（循环依赖的物品价值和）
	public static int sccCnt; // 强连通分量计数器

	public static int[] indegree = new int[MAXN]; // 缩点后DAG中每个SCC的入度

	// 树上背包最优解，来自讲解079，题目5
	public static int[] siz = new int[MAXN]; // 记录树的每个节点的子树大小（按dfn序）
	public static int[] weight = new int[MAXN]; // 按dfn序存储每个节点的重量
	public static int[] value = new int[MAXN]; // 按dfn序存储每个节点的价值
	public static int dfnCnt; // 树的dfn序号计数器
	public static int[][] dp = new int[MAXN][MAXM]; // DP数组，dp[i][j]表示处理到dfn序i，背包容量为j时的最大价值

	public static void addEdge(int a, int b) { // 添加一条从a到b的有向边
		nxt[++cntg] = head[a]; // 新边的next指向a原来的第一条边
		to[cntg] = b; // 新边指向节点b
		head[a] = cntg; // 更新a的头指针指向新边
	}

	public static void tarjan(int u) { // Tarjan算法求强连通分量
	...
	// 缩点后的树，每个节点重新分配dfn序号
	public static int dfs(int u) { // 深度优先搜索，按dfn序重新编号并计算子树大小
	...
	public static int knapsackOnTree() { // 在树上进行01背包DP
	...
	public static void main(String[] args) throws Exception { // 主程序入口
		...
		for (int i = 1; i <= n; i++) { // 读取每个物品的依赖
			depend[i] = in.nextInt(); // 依赖物品编号，0表示无依赖
			if (depend[i] > 0) { // 如果有依赖
