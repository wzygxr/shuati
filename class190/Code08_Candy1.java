...
public class Code08_Candy1 { // 定义公共类Code08_Candy1

	public static int MAXN = 100001; // 定义最大节点数常量，最多100000个人
	public static int MAXM = 200001; // 定义最大边数常量，最多200000条边
	public static int n, k, m; // n为人数，k为要求数，m为实际建边数
	public static int[] a = new int[MAXM]; // 存储建边后的起点数组
	public static int[] b = new int[MAXM]; // 存储建边后的终点数组
	public static int[] c = new int[MAXM]; // 存储建边后的边权数组
	...
	public static int[] weight = new int[MAXM]; // 邻接表的边权数组（表示糖果数量约束）
	...
	public static long[] dp = new long[MAXN]; // 动态规划数组，dp[i]表示SCC i的最少糖果数
	...
	public static boolean condense() { // 缩点操作，构建缩点后的DAG，检查约束是否合法
		cntg = 0; // 重置边计数器
	...
	public static long dpOnDAG() { // 在缩点后的DAG上进行动态规划，求最少糖果数
	...
	public static void main(String[] args) throws Exception { // 主程序入口
		...
		for (int i = 1, op, u, v; i <= k; i++) { // 读取每条要求
			op = in.nextInt(); // 要求类型（1-5）
			u = in.nextInt(); // 第一个人
			v = in.nextInt(); // 第二个人
			if (op == 1) { // u的糖果 == v的糖果
				a[++m] = u; b[m] = v; c[m] = 0; // 添加u->v，边权0（u的糖果 >= v的糖果）
				a[++m] = v; b[m] = u; c[m] = 0; // 添加v->u，边权0（v的糖果 >= u的糖果）
			} else if (op == 2) { // u的糖果 < v的糖果
				a[++m] = u; b[m] = v; c[m] = 1; // 添加u->v，边权1（v的糖果 >= u的糖果+1）
			} else if (op == 3) { // u的糖果 >= v的糖果
				a[++m] = v; b[m] = u; c[m] = 0; // 添加v->u，边权0（u的糖果 >= v的糖果）
			} else if (op == 4) { // u的糖果 > v的糖果
				a[++m] = v; b[m] = u; c[m] = 1; // 添加v->u，边权1（u的糖果 >= v的糖果+1）
			} else { // u的糖果 <= v的糖果
				a[++m] = u; b[m] = v; c[m] = 0; // 添加u->v，边权0（v的糖果 >= u的糖果）
			}
		}
		...
		boolean check = condense(); // 执行缩点操作并检查约束合法性
		if (!check) { // 如果约束无法满足
			out.println(-1); // 输出-1
		} else { // 如果约束可以满足
			long ans = dpOnDAG(); // 在DAG上DP求解最少糖果数
